...
def logPrint(s: object = "", log: _io.TextIOWrapper = log, end: str = "\n", print_time: bool = False, write_time: bool = True) -> None:
    ...
    if print_time:
        print("[%s]%s" %(currentTime_second, s), end = end)
    else:
        print(s, end = end)
    if isinstance(log, _io.TextIOWrapper):
        if write_time:
            log.write("[%s]%s%s" %(currentTime_millisecond, str(s), end))
        else:
...
while True:
    ...
    if resource == "":
        ...
        for root, dirs, files in os.walk(local_prefix):
            if not root in folders_to_delete and len(files) != 0: #请仔细体会后半个条件的作用（Please try to udnerstand the role of the latter condition）
        ...
        for folder in cdragon_folders:
            ...
            if status != 200: #跳过获取异常的文件夹（Skip the folders that fail to be fetched）
                continue
